...
class StartUI extends Application {

  def start(primaryStage: Stage) = {
    primaryStage.setTitle("Конфигурация")

    val selfPortLabel = new Label("Ваш порт: ")
    val selfPortText = new TextField("9000")
    selfPortText.setMaxWidth(80)
    val selfPortError = new Label("")
    selfPortError.setTextFill(Color.RED)
    val seedIpLabel = new Label("Внешний порт: ")
    ...
    val seedPortText = new TextField("9001")
    ...
    val startClusterButton = new Button("Запуск ноды")
    ...
    scene.setOnKeyPressed((e: KeyEvent) => {
      if (e.getCode == KeyCode.ENTER) {
        val configData = immutable.HashMap[String, String]("selfPort" -> selfPortText.getText(),
          "seedIp" -> seedIpText.getText(), "seedPort" -> seedPortText.getText())
        StartCluster.apply(ClusterConfig(configData), primaryStage)
      }
    })
...
object StartUI {
  def main(args: Array[String]) {
    Application.launch(classOf[StartUI], args: _*)
  }
}