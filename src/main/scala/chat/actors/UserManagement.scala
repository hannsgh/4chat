...
trait UserManagement {this: ChatActor =>
  ...
  protected def userManagement: Receive = {
    case Login(userName) => {
      if (getAllUserNames().contains(userName)) {
        ...
        if (authWindow.isShowing) {
          authWindow.hide()
          chatWindow.setTitle("Chat: " + currentName)
          chatWindow.show()
        }
      }
    }
    ...
    case Unlogged(error) => {
      ...
      authWindow.errorRapport.setText(error)
    }
  }
  ...
  def loginCluster(name: String) = {
    ...
    members.size match {
      case 0 => {
        Thread.sleep(800)
        authWindow.errorRapport.setText("Please try again")
      }
